...
Usage example:
    python scripts/run_experiment.py --exp-name test_exp --activation relu --hidden-dim 512 --epochs 20 --wandb
...
class MLP(nn.Module):
    def __init__(self, in_features: int, out_features: int, hidden_sizes: List[int], p_drop=0.2, act_fn: str = "relu"):
        ...
        layers = [nn.Flatten(), nn.Linear(in_features, hidden_sizes[0]), activation, dropout]
        for i in range(len(hidden_sizes) - 1):
            layers += [nn.Linear(hidden_sizes[i], hidden_sizes[i + 1]), activation, dropout]
...
@torch.enable_grad()
def update(network: nn.Module, data: DataLoader, loss: nn.Module, opt: torch.optim.Optimizer) -> list:
    network.train()
    errs = []
...
def fit(network: nn.Module, trainloader: DataLoader, valloader: DataLoader, testloader: DataLoader, epochs: int, lr: float, exp_logger=None):
    ...
    for ep in pbar:
        ...
        if exp_logger is not None:
            exp_logger.log_epoch(epoch=ep + 1, train_loss=epoch_train_loss, val_loss=epoch_val_loss, val_acc=epoch_val_acc)

        print(f"Epoch {ep+1}: train loss: {epoch_train_loss:.4f}, val loss: {epoch_val_loss:.4f}, val acc: {epoch_val_acc*100:.2f}%")
...
class ExperimentLogger:
    def __init__(self, outdir: Path, exp_name: str, config: dict, use_wandb: bool = False, wandb_project: str = "activations"):
        ...
        if use_wandb and wandb is None:
            print("wandb requested but package not installed or failed to import. Continuing without wandb.")

        if self.use_wandb:
            self.wandb_run = wandb.init(project=wandb_project, name=self.exp_name, config=self.config)

    def log_epoch(self, epoch: int, train_loss: float, val_loss: float, val_acc: float):
        rec = {"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "val_acc": val_acc}
    ...
    def finalize(self, best_model: nn.Module, final_test_acc: float, train_losses, val_losses, accuracies):
        # save model
...
def main():
    ...
    parser.add_argument("--activation", type=str, default="relu", choices=["relu", "selu", "exp", "reu", "elu", "softmax"], help="activation to use")
    parser.add_argument("--hidden-dim", type=int, default=512, help="size of first hidden layer (second will be hidden_dim//2)")
    parser.add_argument("--exp-name", type=str, default="DefaultTestRun", help="experiment name for logging and saving")
    ...
    parser.add_argument("--wandb", action="store_true", default=True, help="enable wandb logging (enabled by default)")
    ...
    sys_logger.info(f"MNIST dataset loaded: train={len(train)}, test={len(test)}")
    ...
    trainloader = DataLoader(Subset(train, indices=train_inds), batch_size=args.batch_size, drop_last=True, shuffle=True, num_workers=args.num_workers)
    valloader = DataLoader(Subset(train, indices=val_inds), batch_size=args.batch_size, drop_last=True, shuffle=True, num_workers=args.num_workers)
    testloader = DataLoader(test, batch_size=args.batch_size, drop_last=False, shuffle=False, num_workers=args.num_workers)
    sys_logger.info(f"Dataloaders created: train={len(trainloader.dataset)}, val={len(valloader.dataset)}, test={len(testloader.dataset)}, batch_size={args.batch_size}")

    # model
    hidden_sizes = [args.hidden_dim, max(1, args.hidden_dim // 2)]
    model = MLP(in_features=784, out_features=10, hidden_sizes=hidden_sizes, p_drop=args.p_drop, act_fn=args.activation).to(device)
    sys_logger.info(f"Model initialized: activation={args.activation}, hidden_sizes={hidden_sizes}, p_drop={args.p_drop}")

    # logger
    if args.wandb and wandb is None:
        sys_logger.warning("wandb requested but the package is not installed or failed to import; running without wandb. Install 'wandb' to enable remote logging.")
    outdir = Path(args.outdir) / args.exp_name
    config = vars(args)
    logger = ExperimentLogger(outdir=outdir, exp_name=args.exp_name, config=config, use_wandb=args.wandb, wandb_project=args.wandb_project)
    sys_logger.info(f"Experiment logger initialized at {outdir}. Wandb requested: {args.wandb}; Wandb active: {logger.use_wandb}")

    # run training
    sys_logger.info(f"Starting training for {args.epochs} epochs...")
    train_losses, val_losses, accuracies, test_acc, best_model = fit(model, trainloader, valloader, testloader, epochs=args.epochs, lr=args.lr, exp_logger=logger)
    sys_logger.info(f"Training finished. Test accuracy: {test_acc:.4f}")

    # finalize
    logger.finalize(best_model=best_model, final_test_acc=test_acc, train_losses=train_losses, val_losses=val_losses, accuracies=accuracies)
    sys_logger.info(f"Experiment finalized. Outputs saved to: {Path(outdir).resolve()}")
